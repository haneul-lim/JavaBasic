...
//        System.out.println("프로그램 끝3");
        ...
        try {
            int g = 10, i = 0;
            int j = g / i;      // <- 오류발생!
            System.out.println(j);  // 이 코드는 실행안됨ㅠ
            // 예외가 발생한 부분에서 바로 catch로 넘어가기 때문에 예외 이후의 코드는 실행되지 않음.
        } catch (Exception ex) {
            // 예외상황을 명확하게 지정하지 않음
            ex.printStackTrace();
        }
        ...
        try {
            System.out.println("1 ~ 9 사이 정수를 입력하세요");
            num = sc.nextInt(); // 예외가 발생하는 부분이 try 안에 들어와야함
        ...
        try {
            ...
            // FileWriter fw = new FileWriter((fpath)); 이렇게 한줄로 해줘도 됨
            // 텍스트를 단어단위로 쓰고 읽음(??)
            BufferedWriter bw = new BufferedWriter(fw);
            // 텍스트를 입출력할때 속도를 높이기 위해 사용함
            // 행단위로 쓰고 읽기 때문에 속도가 빠름
            bw.write(sjdata);
            bw.close();
            fw.close();
        } catch (IOException e) {
        ...
        FileWriter fw2 = new FileWriter((fpath));
